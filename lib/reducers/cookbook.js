...
const cookbook = (state = initialState(), action) => {
  switch (action.type) {
    ...
    case Const.SET_RECIPE: {
      const rec = state[Const.RECIPES][action.payload];
      return _.assign({}, state, { [Const.RECIPE]: rec, [Const.VIEW]: Const.RECIPE_VIEW });
    }
    case Const.NEW_RECIPE:
      return _.assign({}, state, { [Const.VIEW]: Const.FORM_VIEW });
    default:
      return state;
  }
};
